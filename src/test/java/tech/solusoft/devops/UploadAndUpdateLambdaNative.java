...
public class UploadAndUpdateLambdaNative {
	...
	public void process() {
		Aws.configure();
	 	...
	 	File file = new File("target/microlam-java17-demo-1.0-SNAPSHOT-aws-lambda-native.zip");
	 	String bucket = Aws.DEPLOYMENT_BUCKET;;
	 	String s3key = S3Utils.uploadFileToS3(file, bucket, 20); //file.getName();//
	 	
	 	LambdaUtils.updateLambdaCode(new String[] {"MicrolamJava17DemoNative"}, bucket, s3key);
	}
}
